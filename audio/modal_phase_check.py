...
for freq in freqs:
    ...
    krs = modes.k
    #print([krs[i+1] - krs[i] for i in range(len(krs)-1)])
    As = modes.phi[0,:]
    ...
    fig = plt.figure()
    plt.suptitle('Compare approximate phase and actual phase over 1km at ' + str(freq) + ' Hz')
    plt.plot(og_phase)
    plt.plot(approx_phase)
    plt.figure()
    plt.suptitle('mean(kr)*r - true phase over a 1km track')
    plt.plot(approx_phase-og_phase)
    plt.savefig('pics/' + str(freq) + 'phase_diff.png')
    plt.clf()

    accumulated_err = og_phase - approx_phase
    print('Accumulated err (true phase - approx phase)', accumulated_err[-1])
    #"""
